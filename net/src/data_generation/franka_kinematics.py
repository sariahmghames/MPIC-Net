...
class FrankaKinematics():
    ...
    def diff_jacobian(self, theta):
        Tee, T_joint = self.fwd_kin( theta )   
        jac = self.geometric_jacobian( T_joint, Tee )
        #print('first jacob=', jac)
        diff_jac = np.gradient(jac, axis=0) # axis = 0
        return diff_jac
